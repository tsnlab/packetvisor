...
use crate::pv::{PvPacket, PvNic};
use pnet::datalink::{interfaces, NetworkInterface, MacAddr};

pub fn gen_arp_response_packet(nic: &PvNic, packet: &mut PvPacket) -> Result<(), ()> {
    ...
    match is_exist {
        Some(target_interface) => {
                unsafe {
                    let src_mac_address: Option<MacAddr> = target_interface.mac;
                    if src_mac_address.is_none() {
                        return Err(())
                    }

                    let src_mac_address: MacAddr = src_mac_address.unwrap();
